...
class SPApartmentSizeBloc extends Bloc<SPApartmentSizeEvent, ApartmentSize> {
  SPApartmentSizeBloc() : super(ApartmentSize(
    oneRoom: Apartment(),
    twoRooms: Apartment(),
    ...
    on<OneRoomFloorAreaPerApartmentChanged>((event, emit) {
      emit(state.copyWith(
          oneRoom: state.oneRoom!.copyWith(floorAreaPerApartment: event.value)));
    });
    on<TwoRoomsFloorAreaPerApartmentChanged>((event, emit) {
      emit(state.copyWith(
          twoRooms: state.twoRooms!.copyWith(floorAreaPerApartment: event.value)));
    });
    on<ThreeRoomsFloorAreaPerApartmentChanged>((event, emit) {
      emit(state.copyWith(
          threeRooms: state.threeRooms!.copyWith(floorAreaPerApartment: event.value)));
    });
    on<FourRoomsFloorAreaPerApartmentChanged>((event, emit) {
      emit(state.copyWith(
          fourRooms: state.fourRooms!.copyWith(floorAreaPerApartment: event.value)));
    });
    on<OneRoomInnerWallsPerApartmentLinearMetersChanged>((event, emit) {
      emit(state.copyWith(
          oneRoom: state.oneRoom!.copyWith(innerWallsPerApartmentLinearMeters: event.value)));
    });
    on<TwoRoomsInnerWallsPerApartmentLinearMetersChanged>((event, emit) {
      emit(state.copyWith(
          twoRooms: state.twoRooms!.copyWith(innerWallsPerApartmentLinearMeters: event.value)));
    });
    on<ThreeRoomsInnerWallsPerApartmentLinearMetersChanged>((event, emit) {
      emit(state.copyWith(
          threeRooms: state.threeRooms!.copyWith(innerWallsPerApartmentLinearMeters: event.value)));
    });
    on<FourRoomsInnerWallsPerApartmentLinearMetersChanged>((event, emit) {
      emit(state.copyWith(
          fourRooms: state.fourRooms!.copyWith(innerWallsPerApartmentLinearMeters: event.value)));
    });
